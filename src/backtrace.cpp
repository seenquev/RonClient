...
boost::function<void()> _exceptionFunction = NULL;
...
{
	struct find_info *data = (find_info*)opaque_data;
	...
	while(StackWalk(IMAGE_FILE_MACHINE_I386,
		...
		if (func == NULL) {
			output_print(ob,"0x%x : %s : %s \n",
				frame.AddrPC.Offset,
				CutDirectory(module_name).c_str(),
				CutDirectory(file).c_str());
		}
		else {
...
extern char* backtrace_output = NULL;
extern LPEXCEPTION_POINTERS exceptionInfo = NULL;
...
exception_filter(LPEXCEPTION_POINTERS info)
{
	...
	output_init(&ob, backtrace_output, BUFFER_MAX);
...
void backtrace_register(boost::function<void()> exceptionFunction)
{
	...
	if (backtrace_output == NULL) {
		backtrace_output = (char*)malloc(BUFFER_MAX);
